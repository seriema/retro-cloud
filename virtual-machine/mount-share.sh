...
##TODO the variables here should come from the rpi, but how? can I use environment variables?
resourceGroupName='secret'
storageAccountName='secret'
storageAccountKey='secret'
fileShareName='secret'
smbPath='//secret.file.core.windows.net/skyscraper-cache'
##
...
if [ -z "$(grep $smbPath\ $mntPath /etc/fstab)" ]; then
    echo "# The changes below were made by retro-cloud" | sudo tee -a /etc/fstab > /dev/null
